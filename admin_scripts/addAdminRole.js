/**
 * Script to add the 'admin' role to an existing fanfaron.
 * Usage: node admin_scripts/addAdminRole.js <surnom_or_email>
 *
 * This will parse the current roles JSON array, append 'admin' if not present,
...
async function addAdminRole(identifier) {
  if (!identifier) {
    console.error('Usage: node admin_scripts/addAdminRole.js <surnom_or_email>');
    process.exit(1);
  }
  ...
  db.get(findSql, [identifier, identifier], (err, row) => {
    ...
    if (roles.includes('admin')) {
      console.info('User already has admin role. No update needed.');
      return shutdown(0);
    }

    roles.push('admin');
    const updatedRolesJson = JSON.stringify(roles);
    ...
    db.run(updateSql, [updatedRolesJson, row.id], function(updateErr) {
      ...
      if (this.changes > 0) {
        console.info(`Successfully added 'admin' role to fanfaron ID ${row.id}.`);
      } else {
