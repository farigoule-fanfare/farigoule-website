...
import iconPresident from "@assets/images/icones-bureau/icon-president.png";
import iconChefmu from "@assets/images/icones-bureau/icon-chefmu.png";
import iconTrez from "@assets/images/icones-bureau/icon-trez.png";
import iconCom from "@assets/images/icones-bureau/icon-com.png";
import iconBiere from "@assets/images/icones-bureau/icon-biere.png";
import iconPrev from "@assets/images/icones-bureau/icon-prev.png";

import "./Portraits.css";

const bureauIcons = {
  president: iconPresident,
  chefmu: iconChefmu,
  trez: iconTrez,
  com: iconCom,
  biere: iconBiere,
...
const Portraits = () => {
  ...
  const instruments = Array.from(
    new Set(fanfarons.map((f) => f.instrument)),
  ).sort();
  const promos = Array.from(new Set(fanfarons.map((f) => f.promo))).sort(
    (a, b) => a - b,
  );
  ...
  const fanfaronsFiltered = fanfarons.filter((fanfaron) => {
    ...
    const byBureau =
      filterBureau === "all" ? true : fanfaron.bureau === filterBureau;
  ...
  return (
    <ContentPageLayout title="Portraits">
      ...
      <div
        className={`portrait-container ${selectedFanfaron ? "withDesc" : "noDesc"}`}
      >
        <div className="portrait-header">
          <h2 className="titre-annuaire" id="annuaire-title">
            L’annuaire des fanfarons
          </h2>
          {fanfarons.length > 20 && (
            <button
      ...
      <div id="filtrerResultats" className="contentPage-form-wrapper">
        <form className="contentPage-form" onSubmit={(e) => e.preventDefault()}>
          <h3 style={{ gridColumn: "1 / -1" }}>Filtrer les résultats</h3>

          <div className="contentPage-form-group">
            <label htmlFor="instrument" className="contentPage-label">
              Instrument :
            </label>
            <select
              id="instrument"
          ...
          <div className="contentPage-form-group">
            <label htmlFor="promo" className="contentPage-label">
              Promo :
            </label>
            <select
              id="promo"
          ...
          <div className="contentPage-form-group">
            <label htmlFor="bureau" className="contentPage-label">
              Bureau :
            </label>
            <select
              id="bureau"
