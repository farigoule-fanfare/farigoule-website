...
const app = express();
const PORT = process.env.SERVER_PORT || 5000; // Use environment variable for port or default to 5000
...
app.listen(PORT, () => {
    ...
    if (db && db.filename) {
        // Check if db object and filename property exist
        console.log(`Connected to SQLite database at: ${db.filename}`);
    } else {
        console.log(
            "SQLite database object not fully initialized or filename not available at server start.",
        );
    }
});
