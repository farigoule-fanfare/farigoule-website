...
const authService = {
    ...
    async login(identifier, password) {
        ...
        const ok =
            fanfaron &&
            (await bcrypt.compare(password, fanfaron.password_hash));
        if (!ok) {
            throw Object.assign(new Error("Identifiants invalides"), {
    ...
    async adminSetPassword(adminId, targetUserId, newPw) {
        ...
        await updatePasswordById(
            targetUserId,
            await bcrypt.hash(newPw, SALT_ROUNDS),
        );
    },
